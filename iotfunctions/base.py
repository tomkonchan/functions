...
class BaseFunction(object):
    ...
    def _calc(self, df):
        ...
        """
        raise NotImplementedError(
            'Class %s is not defined correctly. It should override the _calc() method of the base class.' %
    ...
    def execute(self, df):
        """
        AS calls the execute() method of your function to transform or aggregate data. The execute method accepts a
        dataframe as input and returns a dataframe as output.
        
        If the function should be executed on all entities combined you can replace the execute method wih a custom one
        If the function should be executed by entity instance, use the base execute method. Provide a custom _calc
        method instead.
        """
        group_base = []
    ...
    def _get_arg_metadata(self, isoformat_dates=True):
        ...
        for a in args:
            try:
                ...
                msg = 'Programming error. All arguments must have a corresponding instance variable of the same ' \
                      'name. This function has no instance variable: %s' % a
    ...
    def _getMetadata(self, df=None, new_df=None, inputs=None, outputs=None, constants=None):
        ...
        if df is not None:
            ...
            if len(test_outputs) == 0:
                raise ValueError(
                    'Could not locate output columns in the test dataframe. Check the execute method of the function '
        ...
        for a in args:
            ...
            try:
                ...
                raise AttributeError(
                    'Class %s has an argument %s but no corresponding property. Make sure your arguments and '
                    'properties have the same name if you want to infer types.' % (self.__class__.__name__, a))
            is_array = False
            ...
            # if argument is a number, date or dict it must be a constant
                ...
                if arg_value in df.columns:
                    ...
                    is_added = True
                    msg = 'Non array argument %s exists in the test output dataframe so it is a data item of ' \
                          'type %s' % (a, datatype)
                    logger.debug(msg)
            elif is_array:
            ...
            if is_constant:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                        if is_array:
                            msg = 'Array input %s has no predefined values. It will appear in the UI as a type-in ' \
                                  'field that accepts a comma separated list of values. To set values implement ' \
                                  'the get_item_values() method' % a
                            warnings.warn(msg)
                    else:
                        msg = 'Explicit values were found in the the get_item_values() method for ' \
                              'constant argument %s ' % a
                else:
        ...
        for (array, length) in array_outputs:
            ...
            if array_source is None:
                raise ValueError(
                    'No candidate input array found to drive output array %s with length %s . Make sure input array '
                    'and output array have the same length or explicity define the item_source_array. ' %
                    (array, length))
            else:
    ...
    def _infer_type(self, parm, df=None):
        ...
        for value in parm:
            if is_dict_like(value):
                ...
                if isinstance(value, str):
                    ...
                    raise TypeError(
                        'Cannot infer type of argument value %s for parm %s. Supply a string, number, boolean, '
                        'datetime, dict or list containing any of these types.' % (value, parm))
            else:
        ...
        if datatype is None:
            msg = 'Cannot infer datatype for argument %s. Explicitly set the datatype as LITERAL, BOOLEAN, ' \
                  'NUMBER or TIMESTAMP in the itemDataTypes dict' % parm
            logger.warning(msg)
    ...
    def register(self, df, credentials=None, new_df=None,
        ...
        if not self.base_initialized:
            raise RuntimeError(
                'Cannot register function. Did not call super().__init__() in constructor so defaults have not '
                'be set correctly.')

        if self.category is None:
            raise AttributeError(
                'Class has no category. Class should inherit from BaseTransformer or BaseAggregator to obtain '
        ...
        try:
            ...
            raise ValueError(
                'Unable to register function as local import failed. Make sure it is installed locally '
                'and importable. %s ' % exec_str)
        ...
        if module_url == BaseFunction.url:
            logger.warning(
                'The PACKAGE_URL for your module is the same as BaseFunction url. Make sure that your PACKAGE_URL '
                'points to your own package and not iotfunctions')
        ...
        if credentials is not None:
            msg = 'Passing credentials for registration is preserved for compatibility. Use old style credentials ' \
                  'when doing so, or omit credentials to use credentials associated with the Database object ' \
                  'for the function'
            logger.info(msg)
...
class BaseTransformer(BaseFunction):
    """
    Base class for AS Transform Functions. Inherit from this class when building a custom function that adds
...
class BaseDataSource(BaseTransformer):
    ...
    def get_data(self, start_ts=None, end_ts=None, entities=None):
        '''
        The get_data() method is used to retrieve additional time series data that will be combined with
        existing pipeline data during pipeline execution.
        '''
    ...
    def execute(self, df, start_ts=None, end_ts=None, entities=None):
        ...
        if self.merge_method == 'outer':
            ...
            raise ValueError(
                'Error in function definition. Invalid merge_method (%s) specified for time series merge. '
                'Use outer, concat or nearest')
...
class BaseFilter(BaseTransformer):
    ...
    def filter(self, df):
        ...
        '''
        raise NotImplementedError(
            'This function has no filter method defined. You must implement a custom filter method '
...
class BaseAggregator(BaseFunction):
    """
    Base class for AS Aggregator Functions. Inherit from this class when building a custom function
    that aggregates a dataframe.
    """
...
class BaseDatabaseLookup(BaseTransformer):
    ...
    def __init__(self,
        ...
        if output_items is None:
            # concatenate lookup name to output to make it unique
    ...
    def create_lookup_table(self, df=None, table_name=None):
        ...
        if df is None:
            if self.data is not None:
                ...
                msg = 'Cannot create lookup table as data instance or class variable is not set and ' \
...
class BaseDBActivityMerge(BaseDataSource):
    ...
    This function flattens multiple activity types from multiple activity tables into columns indicating the duration
    of each activity. When aggregating activity data the dimenions over which you aggregate may change during the time
    taken to perform the activity. To make allowance for thise slowly changing dimenions, you may include a customer
    calendar lookup and one or more resource lookups
    '''
    ...
    def get_data(self,
        ...
        for activity, sql in list(self.activities_custom_query_metadata.items()):
            try:
                ...
                logger.warning(
                    'Function attempted to retrieve data for a merge operation using custom sql. There was a '
                    'problem with this retrieval operation. Confirm that the sql is valid and contains column '
                    'aliases for start_date,end_date and device_id')
                logger.warning(sql)
                raise
        ...
        if len(dfs) == 0:
            ...
            try:
                ...
                try:
                    ...
                    msg = 'combine activities requires deviceid, start_date, end_date and activity. supplied ' \
                          'columns are %s' % list(adf.columns)
                    logger.debug(msg)
                    raise
...
class BaseSCDLookup(BaseTransformer):
    ...
    def execute(self, df):
        ...
        try:
            ...
            msg = 'Error looking up scd_property from table %s. Make sure that table name is an scd with ' \
                  'start_data, end_date, deviceid and a property name' % self.table_name
            logger.exception(msg)
            raise
...
class BasePreload(BaseTransformer):
    ...
    They are monitored. Excessive resource consumption will be billed by estimating an equivalent number of
    function executions.
    """
    is_preload = True
    ...
    def evaluate_and_write_model(self, new_model, current_model, db, bucket):
        ...
        if current_model.trained_date != new_model.trained_date:
            if self.greater_is_better and new_model.eval_metric_test > self.acceptable_score_for_model_acceptance:
                ...
                msg = 'Training process did not create a model that passed the acceptance critera. Model ' \
                      'evaluaton result was %s' % new_model.eval_metric_test
                logger.debug(msg)
        if write_model:
